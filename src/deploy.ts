// Adapted from create-mn-app 0.5.1 (Midnight Foundation), Apache-2.0. See NOTICE.
/**
 * Deploy QuietCount contract to a Midnight network (undeployed by default; use --network preview|preprod for public networks).
 *
...
// A separate private-state identifier is used for each contract.
const IS_HELLO_WORLD = process.argv.includes('--hello-world');
const CONTRACT_NAME = IS_HELLO_WORLD ? 'hello-world' : 'counter';
const PRIVATE_STATE_ID = IS_HELLO_WORLD ? 'helloWorldPrivateState' : 'quietCountPrivateState';
const organizerState = loadOrganizerState();
...
console.log('Wallet recovery material is stored locally in .midnight-state.json; it is never printed.');
...
const zkConfigPath = path.resolve(__dirname, '..', 'managed', CONTRACT_NAME);
...
async function createProviders(walletCtx: WalletContext) {
  // Use a locally generated password rather than a shared sample password.
  const privateStatePassword = loadStoragePassword();
  ...
  return {
    privateStateProvider: levelPrivateStateProvider({
      privateStateStoreName: 'quietcount-state',
      accountId,
...
async function main() {
  console.log('\n╔══════════════════════════════════════════════════════════════╗');
  console.log(`║  Deploy QuietCount to ${network}`);
  console.log('╚══════════════════════════════════════════════════════════════╝\n');
  ...
  for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
    try {
      // QuietCount receives the organizer secret only through its private witness.
      deployed = await deployContract(providers, {
        compiledContract: compiledContract as any,
        args: [],
        privateStateId: PRIVATE_STATE_ID,
        initialPrivateState: IS_HELLO_WORLD ? {} : organizerState,
      });
      break;
    } catch (err: any) {
  ...
  fs.writeFileSync(IS_HELLO_WORLD ? 'docs/deployment-hello-world.json' : 'docs/deployment.json', JSON.stringify({ project: IS_HELLO_WORLD ? 'Hello World toolchain smoke check' : 'QuietCount', network, contractAddress, deployer: address.toString(), deployedAt: new Date().toISOString() }, null, 2) + '\n');
  console.log('  Saved to .midnight-state.json\n');

  await persistWalletState(network, walletCtx);
  await walletCtx.wallet.stop();
  console.log('─── Deployment complete ────────────────────────────────────────\n');
  console.log('Public deployment evidence saved under docs/. Keep the ignored wallet and organizer files private.\n');
}
